...
1. **Run the Backend:** Visit the ⚙️ [Backend API Server](https://github.com/RPriago/backend_MechineLearning_PI) and follow the `README.md` instructions there to start the FastAPI server on `localhost:8000`.
2. **Run the Frontend:** Visit the 💻 [Frontend Client App](https://github.com/RPriago/frontend_MechineLearning_PI) and run `npm start` to open the camera interface in your browser.
3. *(Optional)* **Explore the Model:** Visit the 🧠 [Model & Training Engine](https://github.com/RPriago/SignLanguage_Project) if you wish to experiment with the dataset or retrain the model with new parameters.
